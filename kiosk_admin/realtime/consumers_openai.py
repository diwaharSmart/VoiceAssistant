...
class RealtimeConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        initial ="""[
                    {
                        "speech_response": self.welcome_text
                    },
                    {
                        "cart_items": [],
    ...
    async def send_final_transcript(self, transcript_text):
        ...
        if transcript:
            ...
            try:
                #thread
                await self.update_cart(json.loads(output)[1]["cart_items"])
            ...
            try:
                speech_audio = await self.convert_text_to_speech(json.loads(output)[0]["speech_response"].replace("cart_total",convert_currency_to_words(self.cart.total)))
