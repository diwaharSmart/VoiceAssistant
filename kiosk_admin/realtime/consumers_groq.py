...
import re
speech_pattern = r'"speech_response":\s*["]?([^",]+)["]?'  # Handles quoted and unquoted strings
cart_items_pattern = r'"cart_items":\s*(\[[^\]]+\])'

def convert_currency_to_words(amount):
    """
    Convert a currency amount in USD to words, up to thousands.
    Supports two decimal places for cents.

    Parameters:
        amount (float): The amount to convert (e.g., 1234.56).

    Returns:
        str: The amount in words.
    """
    if amount < 0 or amount >= 1000000:
        return "Amount must be between 0 and 999,999.99 USD."
    ...
    dollars_text = f"{num2words(dollars, lang='en')} dollar{'s' if dollars != 1 else ''}"
    cents_text = f"{num2words(cents, lang='en')} cent{'s' if cents != 1 else ''}" if cents > 0 else ""
...
class RealtimeConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        self.open_ai_client = Groq(api_key="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")
        ...
        self.chat_history.append(system_instruction)

        initial ="""[
                    {
                        "speech_response": """+str(self.welcome_text)+"""
                    },
                    {
                        "cart_items": [],
    ...
    async def send_final_transcript(self, transcript_text):
        ...
        if transcript:
            ...
            cart_items_match = re.search(cart_items_pattern, output)
            cart_items = json.loads(cart_items_match.group(1)) if cart_items_match else []
            print(cart_items)
            speech_response_match = re.search(speech_pattern, output)
            speech_response = speech_response_match.group(1).strip() if speech_response_match else ""
            try:
                #print(json.loads(output)[1]["cart_items"])
                #thread
                await self.update_cart(cart_items)
            ...
            try:
                # print(json.loads(output)[0]["speech_response"])
                speech_audio = await self.convert_text_to_speech(speech_response.replace("cart_total",convert_currency_to_words(self.cart.total)))
    ...
    async def update_cart(self, new_items):
        ...
        for item in new_items:
            ...
            product = await sync_to_async(Product.objects.get)(id=str(product_id))
            # Create a new CartItem
            cart_item = CartItem(cart=self.cart, product=product, quantity=int(quantity),instructions=instruction)
